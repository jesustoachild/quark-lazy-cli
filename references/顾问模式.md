...
`LAZY_CLI_ADVISOR=code` 下，`--add-prompt` 支持 `kword=`，用于在代码规则选出分享链接后，再按分享链接标题做硬过滤。没有 `kword=` 时，`code` 顾问行为不变。

```bash
LAZY_CLI_ADVISOR=code qslazy update all 大唐迷雾 \
  --add-prompt "kword=臻彩MAX+"
```

语法：

- `|` 表示 OR，命中任意一组即可保留。
- `&` 表示 AND，同一组内关键词必须全部命中。
- 匹配忽略空格和大小写。
- 只过滤分享链接标题，不过滤目录里的文件名。

示例：

| 目标 | `--add-prompt` 示例 |
|------|---------------------|
| 只保留臻彩 MAX+ | `kword=臻彩MAX+` |
| OR：两种写法任一命中 | `kword=臻彩MAX+|臻彩 MAX+` |
| AND：同时命中 4K 和臻彩 MAX+ | `kword=4K&臻彩MAX+` |
| OR + AND | `kword=臻彩MAX+&更新13集|臻彩MAX+&更至EP13` |

过滤后，CLI 会把最新集数收敛到保留下来的分享链接标题。例如搜索结果里普通 4K 已更新到 14 集，但 `臻彩MAX+` 只更新到 13 集，使用 `kword=臻彩MAX+` 后，本次目标会按 13 集判断，避免用被过滤掉的 14 集资源污染目标集。

Debug 示例：

```text
[add_prompt kword过滤] groups=[['4k', '臻彩max+']] 顾问已选分享链接: 7 -> 2，保留编号=[1, 7]，max_ep: 14 -> 14
```
