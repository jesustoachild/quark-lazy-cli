...
class HumanQasAdvisor(QasAdvisor):
    ...
    def advice_search_result(self, ctx: SearchContext) -> AdvisorDecision:
        ...
        print(
            f"\n  🔍 搜索到 {len(fresh)} 个{self.FRESH_DAYS}天内新鲜资源"
            + (f"，{len(old)} 个旧资源" if old else "")
        )
        ...
        stop = (
            ...
            if fresh
            else (print(f"\n--- ⚠️ 无 {self.FRESH_DAYS} 天内新鲜资源 ---") or False)
        )
        if not stop:
        ...
        if not stop and old:
            if (
                input(
                    f"\n👉 是否继续查看{len(old)}个{self.FRESH_DAYS}天以上的旧资？(y/n) [n]: "
                )
                .strip()
                .lower()
                == "y"
...
============================================================
🔍 搜索到 {len(fresh_items)} 个{self.FRESH_DAYS}天内新鲜资源
注意：由于分享资源经常失效，请尽量选择3-5个符合条件的新鲜资源，放入selected字段，如[1, 2, 3, 5]
