...
class HumanQasAdvisor(QasAdvisor):
    ...
    def advice_search_result(self, ctx: SearchContext) -> AdvisorDecision:
        ...
        if ctx.fresh_invalid_count > 0:
            fresh_label = f"搜索到 {self.FRESH_DAYS} 天内新鲜资源：{fresh_total} 个，验证资源有效后：{len(fresh)} 个（其中 {ctx.fresh_invalid_count} 个失效）"
        else:
            fresh_label = f"搜索到 {self.FRESH_DAYS} 天内新鲜资源：{fresh_total} 个，验证资源有效后：{len(fresh)} 个"
        print(f"\n  🔍 {fresh_label}")
        ...
        def show_batch(target: list[dict], phase: str) -> bool:
        ...
        stop = (
            show_batch(fresh, "新鲜资源")
            if fresh
            else (print(f"\n--- ⚠️ 无可选有效新鲜资源 ---") or False)
        )
        if not stop:
        ...
        if not stop and old:
            if (
                input(
                    f"\n👉 是否继续查看{len(old)}个{self.FRESH_DAYS}天以上、验证资源有效后的旧资源？(y/n) [n]: "
                )
                .strip()
                .lower()
                == "y"
    ...
    def advice_search_result(self, ctx: SearchContext) -> AdvisorDecision:
        ...
        if self.debug:
            ...
            for i, item in enumerate(raw):
                ...
                print(f"  [{i + 1}] {fresh_mark} | EP{ep} | {title[:45]}")
...
class LLMQasAdvisor(QasAdvisor):
    ...
    def advice_search_result(self, ctx: SearchContext) -> AdvisorDecision:
        if self.debug:
            sys.stderr.write(
                f"\n[LLM Advisor] 收到搜索结果: {len(ctx.search_results)} 条\n"
            )
        fresh_items, _ = self._split_fresh_old(ctx.search_results)
        if self.debug:
            sys.stderr.write(f"[LLM Advisor] 7天内新鲜资源: {len(fresh_items)} 条\n")
        if not fresh_items:
            return AdvisorDecision()
        ...
        if ctx.fresh_invalid_count > 0:
            fresh_label = f"搜索到 {self.FRESH_DAYS} 天内新鲜资源：{fresh_total} 个，验证资源有效后：{len(fresh_items)} 个（其中 {ctx.fresh_invalid_count} 个失效）"
        else:
            fresh_label = f"搜索到 {self.FRESH_DAYS} 天内新鲜资源：{fresh_total} 个，验证资源有效后：{len(fresh_items)} 个"
        ...
        llm_fresh_label = f"搜到 {self.FRESH_DAYS} 天内有效的资源：{len(fresh_items)} 个"
...
{{"selected": [1, 2, 3, 5], "max_ep": 19}}"""
        try:
            ...
            # 解析 selected（1-based转0-based），过滤超出范围的值
            raw_selected = parsed.get("selected", [])
            if raw_selected:
                selected = [
                    int(n) - 1 for n in raw_selected if 0 < n <= len(fresh_items)
                ]
            else:
                selected = list(range(len(fresh_items)))  # LLM没返回时默认全部
            if self.debug:
                sys.stderr.write(
                    f"[LLM 解析结果] selected={selected}, quark_userlocal_max_ep={quark_userlocal_max_ep}\n"
                )
            return AdvisorDecision(
...
class AgentQasAdvisor(QasAdvisor):
        ...
        for _ in range(self.MAX_TRIES):
            ...
            if os.path.exists(decision_file):
                try:
                    ...
                    selected, max_ep = self._validate(data, len(raw))
                    return AdvisorDecision(
                        advisor_selected_urls_index=sorted(selected),
    ...
    def _validate(self, data: dict, total: int) -> tuple[list, int]:
        raw_selected = data.get("selected", [])
        selected = [n - 1 for n in raw_selected if 0 < n <= total]
