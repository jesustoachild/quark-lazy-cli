...
class QasLazyCli:
    ...
    def task_status(self, identifier):
        ...
        for tid in target_ids:
            ...
            # 扫描本地（内联逻辑，避免调用已删除的 standalone 函数）
            ...
            if returned_paths:
                ...
                if actual_dir != target_dir:
                    ...
                    return
            owned_eps, ep0_files, dup_eps, unmatch_files = set(), [], [], []
            min_ep, max_ep, max_ep_mtime = 9999, 0, 0
            ...
            for f in all_files:
                ...
                if match:
                    ...
                    if ep <= 0:
                        ep0_files.append((fname, f.get("size", 0)))
                        continue
                    owned_eps.add(ep)
                    if ep > max_ep:
                        max_ep = ep
                        max_ep_mtime = f.get("updated_at", 0) or f.get("mtime", 0)
                    if ep < min_ep:
                        min_ep = ep
            min_ep = min_ep if min_ep != 9999 else 1

            holes = sorted([e for e in range(min_ep, max_ep + 1) if e not in owned_eps and e > 0])
            quark_userlocal_min_ep = min_ep
            quark_userlocal_max_ep = max_ep
            max_ep_mtime = max_ep_mtime

            # 搜索资源（复用公共逻辑）
            try:
                unique_results, _ = self._get_refined_search_results(normalized_name)
            except Exception as e:
                print(f"⚠️  搜索超时或网络错误，跳过：{e}")
                return

            # 分类新鲜/旧资源（不切片，与 update 行为一致）
            ...
            holes_str = (
                "无缺集"
                if not holes
                else f"缺{list(holes)[:5]}{'...' if len(holes) > 5 else ''}"
            )

            disk_info = f"最高 {quark_userlocal_max_ep}"
            if max_ep_mtime:
                disk_info += f"（更新于：{simple_relative_day(max_ep_mtime)}）"
            disk_info += f"，最低 {quark_userlocal_min_ep}，{holes_str}"

            print(f"""
============================================================
  📋 任务：{taskname}
  💾 网盘：{disk_info}
  🎯 目标：第{quark_userlocal_max_ep}集以上资源
  💎 偏好：{"最高画质优先" if policy == "prefer_quality" else "最大体积优先"}

============================================================
  🔍 搜索到 {len(fresh)} 个{fresh_days}天内新鲜资源，{len(old)} 个旧资源
""")
    ...
    def _scan_quark_userlocal_directory(self, ctx) -> None:
        """扫描本地目录，结果写入 ctx 属性"""
        save_res = self.client.get_savepath_detail(path=ctx.task_config_profile["savepath"])
        returned_paths = save_res.get("data", {}).get("paths", [])
        if returned_paths:
            actual_dir = returned_paths[-1].get("name", "")
            target_dir = ctx.task_config_profile["savepath"].rstrip("/").split("/")[-1]
            if actual_dir != target_dir:
        ...
        filename_prefix = ctx.task_config_profile["filename_prefix"]
        (
            quark_userlocal_owned_eps_set,
            quark_userlocal_eps_count,
        ) = set(), {}
        ep0_files = []
        quark_userlocal_min_ep, quark_userlocal_max_ep = 9999, 0
        max_ep_mtime = 0
        all_files = save_res.get("data", {}).get("list", [])
        ...
        for f in all_files:
            ...
            if match:
                ep = int(match.group(1))
                if ep <= 0:
                    ep0_files.append((fname, fsize))
                    continue
                quark_userlocal_owned_eps_set.add(ep)
                quark_userlocal_eps_count[ep] = quark_userlocal_eps_count.get(ep, 0) + 1
                if ep > quark_userlocal_max_ep:
                    quark_userlocal_max_ep = ep
                    max_ep_mtime = f.get("updated_at", 0) or f.get("mtime", 0)
                elif ep == quark_userlocal_max_ep:
                    mtime = f.get("updated_at", 0) or f.get("mtime", 0)
                    if mtime > max_ep_mtime:
                        max_ep_mtime = mtime
                if ep < quark_userlocal_min_ep:
                    quark_userlocal_min_ep = ep

        quark_userlocal_min_ep = (
            quark_userlocal_min_ep if quark_userlocal_min_ep != 9999 else 1
        )
        dup_eps = [ep for ep, cnt in quark_userlocal_eps_count.items() if cnt > 1]
        unmatch_files = [
            f.get("file_name", "").strip()
            for f in all_files
            if f.get("file_name", "").strip()
            and (
                not filename_prefix
                or not f.get("file_name", "").strip().startswith(filename_prefix)
            )
        ]
        quark_userlocal_hole_eps_set = sorted(
            [
                e
                for e in range(quark_userlocal_min_ep, quark_userlocal_max_ep + 1)
                if e not in quark_userlocal_owned_eps_set and e > 0
            ]
        )

        # 更新报告缓存
        ctx.snapshot_before = {
            "max": quark_userlocal_max_ep,
            "min": quark_userlocal_min_ep,
            "hole": quark_userlocal_hole_eps_set,
            "owned": quark_userlocal_owned_eps_set,
            "last_updated": max_ep_mtime,
        }

        ctx.max_ep_mtime = max_ep_mtime
        ctx.quark_userlocal_owned_eps_set = quark_userlocal_owned_eps_set
        ctx.quark_userlocal_min_ep = quark_userlocal_min_ep
        ctx.quark_userlocal_max_ep = quark_userlocal_max_ep
        ctx.ep0_files = ep0_files
        ctx.dup_eps = dup_eps
        ctx.unmatch_files = unmatch_files
        ctx.quark_userlocal_hole_eps_set = quark_userlocal_hole_eps_set
        ctx.all_files = all_files

    def _get_refined_search_results(self, pure_name: str) -> tuple[list, bool]:
        """
        公共搜索与过滤逻辑。
        返回 (unique_results, from_urls_predicted_latest_ep)
        """
        ...
        return unique_results, from_urls_predicted_latest_ep
    ...
    def _get_focused_urls_via_search(self, ctx) -> None:
        ...
        unique_results, from_urls_predicted_latest_ep = (
    ...
    def _bfs_build_resource_pool(self, ctx) -> list:
        ...
        for i, current_url in enumerate(ctx.focused_urls_list):
            ...
            if settings.debug:
                ...
                for f in res.get("data", {}).get("list", []):
                    ...
                    if fmtime:
                        ...
                        mtime_str = ""
                    sys.stderr.write(
                        f"  {marker} {name[:30]} | re={fn_re[:15]} | saved={fn_saved[:15]} | {_format_size(fsize)} | {mtime_str}\n"
                    )
            ...
            root_eps, root_score, root_info, root_fm, root_ep_count = self._evaluate_files_for_target(
                ctx,
                res.get("data", {}).get("list", []),
                current_url,
                show_debug=True,
            )
            all_eps, best_score, best_res = set(root_eps), root_score, res
            ep_first_found = {ep: {**root_info[ep], **root_fm[ep]} for ep in root_eps}
            merged_ep_count = dict(root_ep_count)
            url_ep_counts = {current_url: len(root_eps)}

            valid_node_count = 0  # noqa: F841
            valid_node_keys = []  # 收集有效节点编号
            for node_url, node_data in all_nodes.items():
                if node_url == current_url:
                    continue  # 根节点已在 evaluate_files 处理过
                ...
                if settings.debug:
                    ...
                    node_name = node_data.get("dir_name", "?")
                ...
                if node_max_ep < ctx.advisor_confirmed_max_ep:
                    continue  # 无有效 EP，跳过
                valid_node_count += 1
                valid_node_keys.append(node_data.get("node_key", "?"))
                node_eps, node_score, node_info, node_fm, node_ep_count = self._evaluate_files_for_target(
                ctx,
                file_list,
                node_url,
                show_debug=False,
            ...
            if settings.debug:
                root_valid = len(root_eps) > 0
                all_valid_keys = [str(i)] + valid_node_keys if root_valid else valid_node_keys
                if not valid_node_keys and not root_valid:
                    sys.stderr.write(f"[_bfs_build_resource_pool] BFS Step2: 候选URL {i} 无有效子目录（max_ep 均未达标）\n")
                else:
                    for nk in all_valid_keys:
                        sys.stderr.write(
                            f"[_bfs_build_resource_pool] BFS Step2: 候选URL {i}，有效节点编号: {nk}，EP: {sorted(all_eps)} → 已加入候选池\n"
                        )
            if all_eps:
                pool.append(
                    {
    ...
    def _execute_phase1_phase2_quark_auto_save(self, ctx) -> set:
        ...
        for url_idx in range(len(ctx.bfs_result_pool)):
            if not remaining_regex_or_exact_batch_save_candidates:
                break

            pool_url = ctx.bfs_result_pool[url_idx]["url"]
            ...
            if settings.debug:
                sys.stderr.write(
                    f"\n\n[QAS转存Phase1-批量转存DEBUG] url_idx={url_idx} pool_url={pool_url[-40:]} batch={sorted(batch_eps)} 待批量转存队列={sorted(remaining_regex_or_exact_batch_save_candidates)}\n"
                )

            exact_parts = []
            for ep in batch_eps:
                exact_regex = self.generate_exact_filename_regex(ep, ctx.bfs_result_pool[url_idx])
                if exact_regex:
                    exact_parts.append(exact_regex)
                else:
                    exact_parts.append(
                        self.ep_filename_regex_template.replace("{ep}", f"(0*{ep})")
                    )
            smart_pattern = "|".join(exact_parts)

            if settings.debug:
                self.logger.info(
                    f"\n🚀 [QAS转存Phase1-批量转存DEBUG] 调用 run_script_now: EP {sorted(batch_eps)} via {pool_url[-40:]}"
                )
                self.logger.info(f"🎬 夸克转存路径: {pool_url}")
                self.logger.info(f"🧪 匹配正则: {smart_pattern}")
            ...
            if settings.debug:
                sys.stderr.write(
                    f"\n\n[QAS转存Phase1-批量转存DEBUG] actual={sorted(actual_saved_eps)} 待批量转存队列(remaining_regex_or_exact_batch_save_candidates)={sorted(remaining_regex_or_exact_batch_save_candidates) if remaining_regex_or_exact_batch_save_candidates else '已清空，候选剧集全部转存'}\n"
                )

        if settings.debug:
            self.logger.info(
                f"[QAS转存Phase1-批量转存DEBUG] 转存完成 已完成转存(actual_saved_eps_set): {sorted(actual_saved_eps_set)}, 待批量转存队列(remaining_regex_or_exact_batch_save_candidates)={sorted(remaining_regex_or_exact_batch_save_candidates) if remaining_regex_or_exact_batch_save_candidates else '已清空，候选剧集全部转存'}"
            )
        ...
        for item in ctx.exact_one:
            ...
            exact_pattern = self.generate_exact_filename_regex(ep, pool_entry)
            if not exact_pattern:
                exact_pattern = self.ep_filename_regex_template.replace("{ep}", f"(0*{ep})")
            url = pool_entry["url"]
    ...
    def _do_smart_update_single(
        ...
        if not ctx.bfs_result_pool:
            raise Exception(
                f"NOT_FAILED_JUST_NO_UPDATE|{ctx.task_config_profile['original_shareurl_ban'] or 'No valid episodes'}"
            )
    ...
    def _save_report(self, run_results):
        ...
        status_map = {"success": "成功", "no_update": "无更新", "failed": "失败"}
        msg_map = {
        ...
        result_emoji = "✨" if status_cn == "成功" else "🔕"
    ...
    def _log_task_header(self, ctx) -> None:
        ...
        ep0_str = (
            f" ⚠️  发现 {len(ctx.ep0_files)} 个无效 EP0 文件"
            if ctx.ep0_files
            else ""
        )
        unmatch_str = (
    ...
    def _handle_update_command(
        ...
        try:
            ...
            try:
                ...
                status = "no_update" if res.get("no_update") else "success"
                run_results.append(
                    {
                        "taskname": task.get("taskname"),
                        "status": status,
                ...
                err = str(e)
                status = (
                    "no_update" if "NOT_FAILED_JUST_NO_UPDATE|" in err else "failed"
                )
                run_results.append(
                    {
                        "taskname": task.get("taskname"),
