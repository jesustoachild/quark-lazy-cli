...
""")
    ...
    def _recompute_planned_save_eps_after_pool(self, ctx) -> set:
        ...
        # 职责单一化：只做交集（target_save_eps_set ∩ pool_eps），不过问 update_mode
        # update_mode 职责已迁移到 _build_target_save_eps_and_selected_regex()
        all_pool_eps = set()
    ...
    def _execute_phase1_phase2_quark_auto_save(self, ctx) -> set:
        ...
        for cand_url, ep_cand_list in batch_by_url.items():
            ...
            if settings.debug:
                sys.stderr.write(f"[Phase1 DEBUG] replace={target_task_run['replace']}\n")
            ...
            try:
                ...
                # Phase1 失败也恢复内存 ctx.target_task
                ctx.target_task.update({
                ...
                if settings.debug:
                    sys.stderr.write(f"[Phase1 DEBUG] ctx.target_task restored in memory (exception)\n")
                continue
            ...
            # Phase1 成功后恢复内存 ctx.target_task（finally）
            ctx.target_task.update({
                "pattern": ctx.task_config_profile["original_pattern"],
                "shareurl": ctx.task_config_profile["original_shareurl"],
                "shareurl_ban": ctx.task_config_profile.get("original_shareurl_ban", ""),
            ...
            if settings.debug:
                sys.stderr.write(f"[Phase1 DEBUG] ctx.target_task restored in memory\n")
        ...
        for item in ctx.exact_one:
            ...
            try:
                ...
                if settings.debug:
                    sys.stderr.write(f"[Phase2 DEBUG] ctx.target_task restored in memory\n")
